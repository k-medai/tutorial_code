# 데이터 split 함수
# 출처: https://github.com/snap-stanford/pretrain-gnns/blob/master/chem/splitters.py
import torch
import random
import numpy as np
...
#     train_dataset = [dataset[i] for i in train_idx]  # 리스트 형태로 인덱싱
#     valid_dataset = [dataset[i] for i in valid_idx]
#     test_dataset = [dataset[i] for i in test_idx]

#     # train_dataset = dataset.iloc[torch.tensor(train_idx).tolist(),]
#     # valid_dataset = dataset.iloc[torch.tensor(valid_idx).tolist(),]
#     # test_dataset = dataset.iloc[torch.tensor(test_idx).tolist(),]
...
######## 각 데이터셋의 scaffold를 반환하는 버전 #######
def scaffold_split(dataset, smiles_list, task_idx=None, null_value=0,
                   frac_train=0.8, frac_valid=0.1, frac_test=0.1,
                   # return_smiles=False, 
                   return_scaffolds = False):
    """
    Adapted from https://github.com/deepchem/deepchem/blob/master/deepchem/splits/splitters.py
    Split dataset by Bemis-Murcko scaffolds
    This function can also ignore examples containing null values for a
    ...
    ######  기존 함수에서 scaffolds를 포함하여 반환하도록 변환 #####
    all_scaffold_sets_wsf = [
    ...
    ###### train, valid, test의 scaffold를 각각 저장하는 코드 추가 #####
    train_scf, valid_scf, test_scf = [], [], [] 
    ...
    for scaffold_set_wsf in all_scaffold_sets_wsf:
        scaffold_set = next(iter(scaffold_set_wsf.values()))
        scf = next(iter(scaffold_set_wsf.keys()))
        ...
        if len(train_idx) + len(scaffold_set) > train_cutoff:
            if len(train_idx) + len(valid_idx) + len(scaffold_set) > valid_cutoff:
                ...
                valid_idx.extend(scaffold_set)
                valid_scf.append(scf)
        else:
            train_idx.extend(scaffold_set)
            train_scf.append(scf)
    ...
    ###  return_scaffolds 시 각 dataset의 scaffold를 반환 ###
    # 완성된 scaffold list에서 공백인 항목 거르기
    train_scf = [scf for scf in train_scf if len(scf)!= 0]
    valid_scf = [scf for scf in valid_scf if len(scf)!= 0]
    test_scf = [scf for scf in test_scf if len(scf)!= 0]
    
    # 각 scaffold는 list -> set
    if not return_scaffolds:
        return train_dataset, valid_dataset, test_dataset
    else:
        return train_dataset, valid_dataset, test_dataset, (set(train_scf),set(valid_scf),set(test_scf))
...
def random_scaffold_split(dataset, smiles_list, task_idx=None, null_value=0,
    ...
    ## random_scaffold_split에서 오류 발생 (오류 해결 안 된 상태)
    """
